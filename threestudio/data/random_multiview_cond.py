...
from extern.MVC.mvc.camera_utils import get_camera, create_camera_to_world_matrix, convert_opengl_to_blender


@dataclass
class RandomMultiviewCameraCondDataModuleConfig(RandomCameraDataModuleConfig):
    relative_radius: bool = False
    n_view: int = 1
    zoom_range: Tuple[float, float] = (1.0, 1.0)
    cond_offsets: float = 0.

class RandomMultiviewCameraCondIterableDataset(RandomCameraIterableDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.zoom_range = self.cfg.zoom_range
        self.cond_offsets = self.cfg.cond_offsets
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        for i in range(self.batch_size):
            elevationi = elevation[i]
            azimuth_degi = azimuth_deg[i] + self.cond_offsets
            cond_camera = create_camera_to_world_matrix(elevationi, azimuth_degi)
            cond_camera = convert_opengl_to_blender(cond_camera)

            cond_cameras.append(cond_camera)
        cond_cameras = torch.Tensor(cond_cameras)
        ...
        return {
            ...
            "width": self.width,
            "fovy": fovy_deg,
            "cond_camera": cond_cameras
        }


@register("random-multiview-camera-cond-datamodule")
class RandomMultiviewCameraCondDataModule(pl.LightningDataModule):
    cfg: RandomMultiviewCameraCondDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(RandomMultiviewCameraCondDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = RandomMultiviewCameraCondIterableDataset(self.cfg)
