        ...
        if self.training:
            ...
            if "normal" in geo_out:
                if self.cfg.return_comp_normal:
                    ...
                    w2c = kwargs["c2w"][:, :3, :3].inverse().repeat(comp_normal.shape[0], 1, 1)
                    comp_normal_viewspace = torch.einsum("bij,bj->bi", w2c, comp_normal)
                    bg_normal = torch.zeros_like(comp_normal_viewspace)
                    bg_normal[:, 2:3] = 1.
                    
                    comp_normal_viewspace = F.normalize(comp_normal_viewspace, dim=-1)
                    comp_normal_viewspace = (
                        (comp_normal_viewspace + 1.0) / 2.0 * opacity + (bg_normal + 1.0) / 2.0 * (1 - opacity)
                    )
                    ...
                    out.update(
                        {
                            ...
                            "comp_normal_viewspace": comp_normal_viewspace.view(
                                batch_size, height, width, 3
                            ),
                        }
                    )
