...
@threestudio.register("geoimplicit-sdf")
class GeoimplicitSDF(BaseImplicitGeometry):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        self.sdf_encoding = get_encoding(
            self.cfg.n_input_dims, self.cfg.pos_encoding_config
        )
    ...
    def initialize_shape(self) -> None:
        ...
        if self.cfg.shape_init == "ellipsoid":
            ...
            verts_sdf = mesh.vertices[:, [2,1,0]]
            verts_sdf[:, 1] *= -1.
            sdf = SDF(verts_sdf, mesh.faces)
        ...
        self.get_gt_sdf = get_gt_sdf

        # self.extract_debug(0)
        # explicit broadcast to ensure param consistency across ranks
        for param in self.parameters():
            broadcast(param, src=0)

    def extract_debug(self, id):
        import mcubes
        import trimesh
        bound_min = [-1., -1., -1.]
        bound_max = [1., 1., 1.]
        resolution = 256
        threshold = 0
        query_func = lambda pts: -self.forward_sdf(pts)
        u = extract_fields(bound_min, bound_max, resolution, query_func)
        vertices, triangles = mcubes.marching_cubes(u, threshold)
        b_max_np = np.array(bound_max)
        b_min_np = np.array(bound_min)
        vertices = vertices / (resolution - 1.0) * (b_max_np - b_min_np)[None, :] + b_min_np[None, :]
        debug_mesh = trimesh.Trimesh(vertices=vertices, faces=triangles)
        debug_mesh.export("debug_%d.obj" % id)
    ...
    def forward(
        ...
        enc = self.sdf_encoding(points.view(-1, self.cfg.n_input_dims))
    ...
    def forward_sdf(self, points: Float[Tensor, "*N Di"]) -> Float[Tensor, "*N 1"]:
        ...
        points = contract_to_unisphere(points_unscaled, self.bbox.to(points.device), self.unbounded)

        sdf = self.sdf_network(
            self.sdf_encoding(points.reshape(-1, self.cfg.n_input_dims))
    ...
    def forward_field(
        ...
        enc = self.sdf_encoding(points.reshape(-1, self.cfg.n_input_dims))
