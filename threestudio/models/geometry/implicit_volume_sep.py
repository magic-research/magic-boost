...
@threestudio.register("implicit-volume-sep")
class ImplicitVolumeSep(BaseImplicitGeometry):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        if self.cfg.fix_geometry:
            self.geo_encoding = get_encoding(
            self.cfg.n_input_dims, self.cfg.pos_encoding_config
            )
    ...
    def forward(
        ...
        if self.cfg.fix_geometry:
            enc = self.geo_encoding(points.view(-1, self.cfg.n_input_dims))
            density = self.density_network(enc).view(*points.shape[:-1], 1).detach()
            color_enc = self.encoding(points.view(-1, self.cfg.n_input_dims))
        else:
        ...
        if self.cfg.n_feature_dims > 0:
            features = self.feature_network(color_enc).view(
